#!/usr/bin/env python3
"""
Fine-tune Mistral (or compatible chat models) directly from a JSON dataset
...
def build_collator(processor):
    ...
    locating the response marker from the model's chat template. For Mistral,
    the marker '[/INST]' is commonly used.
    """

    response_marker = "[/INST]"  # Changed from "<start_of_turn>model\n" for Mistral
...
def get_args():
    ...
    p.add_argument("--model_name", default="mistralai/Mistral-Small-3.1-24B-Instruct-2503")
    p.add_argument(
        "--output_dir", default="./runs_sft_mistral_from_json", help="Output directory"
    )
    ...
    # LoRA - increased for 24B model
    p.add_argument("--lora_r", type=int, default=8)
    p.add_argument("--lora_alpha", type=int, default=16)

    # Quantization
    p.add_argument("--use_4bit", action="store_true")
    p.add_argument("--use_8bit", action="store_true")  # Added 8-bit option for 24B
    ...
    p.add_argument("--convert_to_new_format", action="store_false", 
                   help="Convert legacy image_path format to new message-embedded format")
...
def main():
    ...
    # Convert to new format if requested
    if args.convert_to_new_format:
        if is_main:
    ...
    control_ids = processor.tokenizer.convert_tokens_to_ids(toks)
    if is_main:
    ...
    if not torch.distributed.is_initialized():
        model_kwargs["device_map"] = {"": f"xpu:{rank}" if is_xpu_available() else rank}
    if args.use_4bit:
    ...
    elif args.use_8bit:  # Added 8-bit support
        model_kwargs["quantization_config"] = BitsAndBytesConfig(
            load_in_8bit=True)

    Model = (
        Mistral3ForConditionalGeneration  # Changed from Gemma3ForConditionalGeneration
    ...
    # Removed model.tie_weights() call for Mistral

    # LoRA config with modules_to_save for Mistral
    lora = LoraConfig(
        r=args.lora_r,
        ...
        modules_to_save=["embed_tokens", "lm_head"],  # Added for Mistral
        task_type="CAUSAL_LM",
        auto_mapping=True,
    )
    ...
    targs = TrainingArguments(
        ...
        run_name=f"mistral-from-json-{datetime.now():%Y%m%d-%H%M%S}",  # Changed run name
    ...
    class ShowPredsCallback(TrainerCallback):
        ...
        def on_evaluate(self, args, state, control, **kwargs):
            ...
            device = model.device
            END_ID = tokenizer.convert_tokens_to_ids("</s>")  # Changed from "<end_of_turn>" for Mistral
    ...
    trainer = SFTTrainer(
        ...
        callbacks=[cb,early_stop_cb]
    )

    # Note: embed_tokens and lm_head are automatically trainable due to modules_to_save in LoRA config
    ...
    if is_main:
        ...
        # Removed weight tying check for Mistral since it doesn't use tie_weights()
    ...
    # Save final model (no LoRA merging for Mistral)
    if is_main:
        print("\n" + "=" * 60)
